...
import { PROMOTERS as STATIC_PROMOTERS, OBJECTIVES } from '../constants';
...
const About: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch Promoters from Backend API
    const fetchPromoters = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          ...
          setTeam(STATIC_PROMOTERS); // Fallback to constant
        }
      } catch (error) {
        console.error("Failed to fetch promoters:", error);
        setTeam(STATIC_PROMOTERS);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center">
          ...
          <div className="text-emerald-600 font-bold text-sm tracking-widest uppercase">Loading Core Team...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-20 pb-16 bg-slate-50 min-h-screen">
      ...
      {/* Leadership Section - Linked to Backend API */}
                ...
                <div className="p-8 md:p-10 flex-grow flex flex-col items-center relative z-10">
                  <div className="w-32 h-32 md:w-40 md:h-40 rounded-3xl overflow-hidden mb-6 md:mb-8 border-4 border-slate-50 group-hover:border-emerald-100/50 group-hover:scale-105 transition-all duration-700 shadow-xl relative bg-slate-50">
                    ...
                    <img 
                      ...
                      style={{ objectPosition: p.imagePosition || p.image_position || '50% 50%' }}
                      alt={p.name} 
                      loading="lazy" 
                    />
                  </div>
                  ...
                  <div className="mt-auto w-full pt-6 border-t border-slate-50 group-hover:border-emerald-50 transition-colors flex flex-wrap justify-center gap-2">
                    {/* Safely map specialties array to avoid crashes */}
                    {(p.specialties || []).map((s: string, index: number) => (
                      <span key={index} className="px-2.5 py-1 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-wider rounded-md border border-slate-200 group-hover:bg-emerald-50 group-hover:text-emerald-700 group-hover:border-emerald-200 transition-all duration-500">
                        {s}
                      </span>
                    ))}
                  </div>
