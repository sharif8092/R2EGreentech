...
// Helper for dynamic icons based on Database Slug
...
const IndustryVisualCard: React.FC<{ industry: any, image: string, position: string, icon: any }> = ({ industry, image, position, icon }) => {
      ...
      <div className="relative z-10 flex gap-4 md:gap-6 items-center">
        <div className="shrink-0">
          <div className="w-12 h-12 md:w-14 md:h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center border border-slate-100 group-hover:bg-emerald-600 group-hover:text-white group-hover:border-emerald-500 group-hover:rotate-6 group-hover:scale-110 group-hover:shadow-sm group-hover:shadow-emerald-500/40 group-[.mobile-active]:bg-emerald-600 group-[.mobile-active]:text-white group-[.mobile-active]:border-emerald-500 group-[.mobile-active]:rotate-6 group-[.mobile-active]:scale-110 group-[.mobile-active]:shadow-sm group-[.mobile-active]:shadow-emerald-500/40 transition-all duration-500">
            ...
            {!['HVAC', 'Solar', 'E-Waste', 'Quality', 'Engineering'].includes(sol.vertical) && <Building2 className="w-5 h-5 md:w-6 md:h-6" />}
          </div>
        </div>
...
const Industries: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // API Fetch Logic (Backend Integration)
    const fetchData = async () => {
      try {
        const [indRes, settingsRes] = await Promise.allSettled([
        ...
        if (indRes.status === 'fulfilled' && indRes.value.data && indRes.value.data.length > 0) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center">
          ...
          <div className="text-emerald-600 font-bold text-sm tracking-widest uppercase">Loading Industries...</div>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-24 md:space-y-32">
            {industries.map((industry: any, idx) => {
              const indId = industry.id || industry.slug;
              
              // Dynamic Image & Icon mapping from DB and Site Settings
              const dynamicImage = siteSettings[`img_industry_${indId}`] || industry.image || "https://via.placeholder.com/600";
              const dynamicPos = siteSettings[`pos_industry_${indId}`] || industry.imagePosition || '50% 50%';
              const dynamicIcon = getIcon(indId);

              return (
                <div key={indId || idx} className="grid lg:grid-cols-12 gap-10 md:gap-12 items-start">
                  {/* Visual Sidebar */}
